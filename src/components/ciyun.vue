<template>
  <div class="ciyun-container">
    <div id="ciyun" class="ciyun"></div>
    <div class="info">
      <p>双击地图坐标可以查看对应类别餐饮的分布(再次双击退出)</p>
      <p>单击查看对应评价词云</p>
    </div>
  </div>
</template>
    ...
    initCiyun(point) {
      d3.json("ciyun.json").then((data) => {
        if (data[point]) {
          ...
          this.$message({ type: "warn", text: "没有数据" });
        }
      });
    },
    drawYun() {
      const doc = document.getElementById("ciyun");
      this.ciyun = this.$echarts.init(doc);
      this.option = {
        title: {
          text: this.title,
        },
        ...
        series: [
          {
            ...
            // height: 200,
            // top: 20,
            data: this.data_ciyun.value,
          },
        ],
      };
      this.ciyun.clear();
      var maskImage = new Image();
      maskImage.src = this.data_ciyun.image;
      maskImage.onload = () => {
        this.ciyun.setOption(this.option, true);
      };
    },
  },
};
